---
title: 甘味飯 再現レシピ
recipeId: sf-sweet-savory-rice
description: Second Floor Cafe 風に 蜜餞、米飯、台南限定 を組み合わせた、店の一皿を意識した再現レシピです。
coverImage: /images/recipes/sf-sweet-savory-rice.webp
servings: 1
prepTime: 20
cookTime: 18
totalTime: 38
difficulty: 普通
calories: 640
protein: 28
fat: 28
carbs: 70
category: "季節限定"
scenarios:
- 一人分の料理
equipment:
- フライパン
- オーブン
tags:
- 名店再現
- 季節限定
restaurantSource:
  restaurant: 貳樓 Second Floor Cafe
  dishName: 甘味飯
  region: 全台連鎖
intro: Second Floor Cafe でよく見られる組み立て方を参考にし、皿全体の流れごと再現するイメージで作ります。
steps:
- 下準備：蜜餞 40gを小さく角切り。ご飯 適量をふっくら炊き保温し、ヘラでほぐす。台南限定 適量を袋の表示どおり切って用意。
- フライパンを弱火にし、オリーブオイル 大さじ1で蜜餞を1分軽く炒め、表面がつやり甘い香りが出るまで（焦がさない）。
- ご飯 適量を加え、中火に上げて2分炒め、米粒の外側に油が回り粒立つまで（炒米）。
- 熱湯を大さじ2ずつ加え、吸われたら次を入れ、2～3回約4分押し混ぜ、米粒がやや湿り粘り気を帯びるまで。
- 海塩 小さじ1/3、挽き黒胡椒 小さじ1/4で甘さを整え、甘辛のバランスを見る。
- 台南限定 適量を加え、1分炒めて甘辛の風味を均一に。
- 火を止め、米粒を潰さないよう軽く和える。
- 温かいうちに丼に盛り、貳樓の台南限定甘味飯の甘辛の層を再現する。
tips:
- 主役の具材とソースは別々に完成させ、最後に盛り付けると食感の差が残せます。
- 冷たい付け合わせと温かい主菜は最後まで分けておくと、店らしい仕上がりになります。
storage: できれば当日中がおすすめ。保存する場合は具材とソースを分けて冷蔵し、食べる前にやさしく温め直します。
substitutions:
- 時蔬
- 白飯
relatedIngredients:
- 蜜餞
- 米飯
- 台南限定
customAdditions: []
faqs:
- question: 甘味飯 を店っぽく仕上げるコツは？
  answer: 主役の具材を先に整え、最後にソースをまとわせてから盛り付けると、レストランらしい厚みが出ます。
featured: false
publishedAt: "2026-06-09"
updatedAt: "2026-07-06"
ingredients:
- name: 蜜餞
  amount: "40"
  unit: 公克
  isCore: true
- name: 米飯
  amount: "適量"
  unit: ""
  isCore: true
- name: 台南限定
  amount: "適量"
  unit: ""
  isCore: true
seasonings:
- name: 橄欖油
  amount: "1"
  unit: 大匙
- name: 海鹽
  amount: "1/3"
  unit: 小匙
- name: 黑胡椒碎
  amount: "1/4"
  unit: 小匙
---

この再現版は、Second Floor Cafe らしいボリューム感と重ねた食感を意識して組み立てています。

主役の具材、ソース、盛り付けの順で仕上げると、店で出てくるような完成度に近づけます。

