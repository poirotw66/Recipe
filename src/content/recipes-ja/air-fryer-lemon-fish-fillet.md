---
recipeId: air-fryer-lemon-fish-fillet
title: エアフライヤーでレモン魚のフィレ
description: 白身魚の切り身をレモンとハーブ風味でエアフライヤー調理。身はしっとり、酸味でさっぱり。一人分にぴったり。
coverImage: /images/recipes/air-fryer-lemon-fish-fillet.webp
servings: 1
prepTime: 8
cookTime: 12
totalTime: 20
difficulty: かんたん
calories: 280
protein: 36
fat: 12
carbs: 4
category: "主菜"
scenarios:
- ノンフライヤー料理
- 一人分の料理
equipment:
- エアフライヤー
tags:
- 一人分
- エアフライヤー
- 魚料理
tips:
- 厚さが2cmを超える場合は2分ほど延ばしてください。フォークでほぐれやすいか必ず確認を。
- 食材の表面を拭いてから一段に並べ、熱風の通り道を空けると均一に焼き色がつきます。
substitutions:
- サーモン
- サバ
relatedIngredients:
- たら
- ブロッコリー
customAdditions: []
ingredients:
- name: たら
  amount: '1'
  unit: 切れ
  isCore: true
- name: ブロッコリー
  amount: '80'
  unit: g
  isCore: false
seasonings:
- name: レモン汁
  amount: '1.5'
  unit: 大さじ
- name: オリーブオイル
  amount: '1'
  unit: 大さじ
- name: にんにく
  amount: '1'
  unit: 片
- name: 塩
  amount: 少々
  unit: ''
- name: 黒胡椒
  amount: 少々
  unit: ''
intro: 魚は加熱しすぎるとパサつきやすいので、時間は短めが安全。中心にしっとり感が残るのがおいしい目安です。
steps:
- たらの切り身1枚をキッチンペーパーで両面の水分をよく拭く。にんにく1片をみじん切りにする。ブロッコリー80gを小房に切り、洗って水気を切り、ペーパーで押さえて乾かす。
- ボウルでレモン汁大さじ1.5、オリーブオイル大さじ1、にんにく、塩少々、黒胡椒を混ぜ、半量を魚の両面に塗り5分おく。
- エアフライヤーを180°Cに設定し、3分予熱する。
- 魚を平らにバスケットに入れ、残りの下味を表面に刷り、1層で重ねない。180°Cで6分加熱。そっと裏返し、さらに4〜5分加熱して表面が軽く焦げるまで。
- フォークで最も厚い部分をほぐすと不透明で簡単に分離すればOK。魚を取り出して保温する。
- バスケットにブロッコリーを入れ180°Cで3〜4分、茎がやや柔らかくまだパリッとするまで（または湯通し1分）。
- 魚とブロッコリーを盛り、お好みでレモン汁を足して熱いうちに。
storage: できたてがおすすめ。魚は冷蔵再加熱で乾きやすいので、できればその日のうちに。
faqs:
- question: バスケットにくっつきますか？
  answer: クッキングシートを敷くか油を刷ればくっつきにくくなります。ひっくり返すときは優しく。
- question: "厚みが違う食材の加熱時間はどう調整しますか？"
  answer: "表示時間で一度確認し、最も厚い部分の火通りを見ます。必要なら2～3分ずつ追加してください。"
featured: false
publishedAt: '2026-06-03'
updatedAt: "2026-07-30"
---

レモンの酸味が爽やかで、外食が多い方でも家で手早く作れる魚のメインです。
