---
title: 珍珠豆腐冰淇淋 再現レシピ
recipeId: dh-tofu-ice-cream-with-tapioca
description: 涓豆腐の 珍珠豆腐冰淇淋 を家庭で再現する、デザート 向けレシピです。
coverImage: /images/recipes/dh-tofu-ice-cream-with-tapioca.webp
servings: 2
prepTime: 15
cookTime: 12
totalTime: 27
difficulty: かんたん
calories: 380
protein: 6
fat: 14
carbs: 56
category: "デザート"
scenarios:
- 一人分の料理
equipment:
- フライパン
- ボウル
tags:
- 名店再現
- デザート
restaurantSource:
  restaurant: 涓豆腐 Dubu House
  dishName: 珍珠豆腐冰淇淋
  region: 全台連鎖
intro: 涓豆腐でよく見られる下ごしらえと仕上げの流れを意識した再現レシピです。
steps:
- 非遺伝子組み換え大豆を混ぜてデザートのベースを準備します（ホットクの生地を作るか、大豆を処理します）。
- フライパンにごま油やバターを熱し、弱火でゆっくりと焼きます（または茹でた大豆をフードプロセッサーで滑らかなアイス状に仕上げます）。
- 黒糖タピオカ（黒糖シナモン堅果餡、または黒糖タピオカパール）を温めて準備します。
- ２つを組み合わせ（生地に餡を包んで平らに焼き上げるか、豆腐アイスに黒糖タピオカをトッピングします）。
tips:
- 調理前に調味料をよく混ぜ合わせておくことで、均一に味が広がります。
- 熱したフライパンと十分な油で素早く炒めることが、水分を逃がさないコツです。
storage: できれば当日中がおすすめ。保存する場合は具材とスープを分けて冷蔵します。
substitutions:
- 非遺伝子組み換え大豆
- 黒糖
relatedIngredients:
- 非基改黃豆
- 黑糖珍珠
customAdditions:
- 刻みネギ
- ごま
faqs:
- question: 珍珠豆腐冰淇淋 を店っぽくするコツは？
  answer: スープやタレの濃度を先に整え、最後に具材を加えるのが近道です。
featured: false
publishedAt: "2026-06-03"
updatedAt: "2026-06-09"
ingredients:
- name: 非遺伝子組み換え大豆
  amount: "150"
  unit: g
  isCore: true
- name: 黒糖タピオカ
  amount: "50"
  unit: g
  isCore: true
seasonings:
- name: 黒糖
  amount: "50"
  unit: g
- name: 塩
  amount: "0.25"
  unit: 少々
---

この再現版は、涓豆腐らしいスープの層とボリューム感を意識して作っています。

