---
title: 起司馬鈴薯煎餅 再現レシピ
recipeId: dh-potato-pancake-with-cheese
description: 涓豆腐の 起司馬鈴薯煎餅 を家庭で再現する、韓国主菜 向けレシピです。
coverImage: /images/recipes/dh-potato-pancake-with-cheese.svg
servings: 2
prepTime: 22
cookTime: 22
totalTime: 44
difficulty: 普通
calories: 640
protein: 30
fat: 26
carbs: 58
category: "韓国主菜"
scenarios:
- 高タンパク料理
equipment:
- フライパン
- 鍋
tags:
- 名店再現
- 韓国主菜
restaurantSource:
  restaurant: 涓豆腐 Dubu House
  dishName: 起司馬鈴薯煎餅
  region: 全台連鎖
intro: 涓豆腐でよく見られる下ごしらえと仕上げの流れを意識した再現レシピです。
steps:
- じゃがいもを細切りまたは粗みじん切りにしておきます。
- ボウルに中力粉、卵、塩、水を混ぜ合わせ、切ったじゃがいも, ハム, ソーセージ, コーン, パプリカを加えて生地を作ります。
- フライパンにごま油を熱し、生地を流し込んで薄く広げます。中火で底がカリッと香ばしくなるまで焼きます。 全体にモッツァレラチーズを散らします。
- 裏返し（チーズを下にして香ばしく焼き上げ）、ごま油を回し入れ、サクサクになるまで両面を焼き上げて切り分けます。
tips:
- 調理前に調味料をよく混ぜ合わせておくことで、均一に味が広がります。
- 熱したフライパンと十分な油で素早く炒めることが、水分を逃がさないコツです。
storage: できれば当日中がおすすめ。保存する場合は具材とスープを分けて冷蔵します。
substitutions:
- じゃがいも
- モッツァレラチーズ
relatedIngredients:
- 馬鈴薯
- 火腿丁
- 德式香腸丁
- 玉米粒
customAdditions:
- 刻みネギ
- ごま
faqs:
- question: 起司馬鈴薯煎餅 を店っぽくするコツは？
  answer: スープやタレの濃度を先に整え、最後に具材を加えるのが近道です。
featured: false
publishedAt: "2026-06-03"
updatedAt: "2026-06-03"
ingredients:
- name: じゃがいも
  amount: "2"
  unit: 個
  isCore: true
- name: ハム
  amount: "50"
  unit: g
  isCore: true
- name: ソーセージ
  amount: "50"
  unit: g
  isCore: true
- name: コーン
  amount: "30"
  unit: g
  isCore: true
- name: パプリカ
  amount: "30"
  unit: g
  isCore: true
seasonings:
- name: モッツァレラチーズ
  amount: "60"
  unit: g
- name: 中力粉
  amount: "150"
  unit: g
- name: 卵
  amount: "1"
  unit: 個
- name: 塩
  amount: "0.25"
  unit: 少々
- name: ごま油
  amount: "1"
  unit: 大さじ
---

この再現版は、涓豆腐らしいスープの層とボリューム感を意識して作っています。

