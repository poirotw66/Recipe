---
title: 酪梨 Local 香腸／丹麥 Open 再現レシピ
recipeId: sf-avocado-local-sausage-danish-open
description: Second Floor Cafe 風に 丹麥麵包、酪梨、香腸、水波蛋、辣椒粉 を組み合わせた、店の一皿を意識した再現レシピです。
coverImage: /images/recipes/sf-avocado-local-sausage-danish-open.webp
servings: 1
prepTime: 18
cookTime: 14
totalTime: 32
difficulty: 普通
calories: 520
protein: 24
fat: 26
carbs: 38
category: "オープンサンド"
scenarios:
- 一人分の料理
- 10分でできる料理
equipment:
- フライパン
- オーブン
tags:
- 名店再現
- オープンサンド
restaurantSource:
  restaurant: 貳樓 Second Floor Cafe
  dishName: 酪梨 Local 香腸／丹麥 Open
  region: 全台連鎖
intro: Second Floor Cafe でよく見られる組み立て方を参考にし、皿全体の流れごと再現するイメージで作ります。
steps:
- 丹麥麵包2份を横切りにし、切り口にバターを薄く塗り、180℃のオーブンで5～7分焼いて表面をきつね色に軽くカリカリにする。
- 香腸140gに数か所切り込みを入れ、フライパンで中弱火6～8分、こまめに返しながら表面をきつね色に中まで火を通す。斜め切りにしておく。
- アボカド1個の種を取り薄切りにする。小さなボウルでオリーブオイル大さじ1、海塩小さじ1/3、黒胡椒小さじ1/4、辣椒粉小さじ1/2を混ぜて調味オイルにする。
- 鍋で湯を沸かし酢を少し加え、弱火にして渦を作り、卵2個を割り入れ約3分ポーチする（白身が固まり黄身がとろり）。
- 焼いた丹麥麵包に調味オイルの半分を塗り、アボカドと香腸を順にのせる。
- ポーチドエッグ2個を載せ、残りの調味オイルをかけて温かいうちに。
tips:
- 主役の具材とソースは別々に完成させ、最後に盛り付けると食感の差が残せます。
- 冷たい付け合わせと温かい主菜は最後まで分けておくと、店らしい仕上がりになります。
storage: できれば当日中がおすすめ。保存する場合は具材とソースを分けて冷蔵し、食べる前にやさしく温め直します。
substitutions:
- 酸種麵包
- 丹麥麵包
relatedIngredients:
- 丹麥麵包
- 酪梨
- 香腸
- 水波蛋
customAdditions:
- ポーチドエッグ追加
- バルサミコ別添え
faqs:
- question: 酪梨 Local 香腸／丹麥 Open を店っぽく仕上げるコツは？
  answer: 主役の具材を先に整え、最後にソースをまとわせてから盛り付けると、レストランらしい厚みが出ます。
featured: false
publishedAt: "2026-06-09"
updatedAt: "2026-07-06"
ingredients:
- name: 丹麥麵包
  amount: "2"
  unit: 份
  isCore: true
- name: 酪梨
  amount: "1"
  unit: 顆
  isCore: true
- name: 香腸
  amount: "140"
  unit: 公克
  isCore: true
- name: 水波蛋
  amount: "2"
  unit: 顆
  isCore: true
seasonings:
- name: 橄欖油
  amount: "1"
  unit: 大匙
- name: 海鹽
  amount: "1/3"
  unit: 小匙
- name: 黑胡椒碎
  amount: "1/4"
  unit: 小匙
- name: 辣椒粉
  amount: "1/2"
  unit: 小匙
---

この再現版は、Second Floor Cafe らしいボリューム感と重ねた食感を意識して組み立てています。

主役の具材、ソース、盛り付けの順で仕上げると、店で出てくるような完成度に近づけます。

