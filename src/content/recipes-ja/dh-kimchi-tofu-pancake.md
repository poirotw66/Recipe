---
title: 泡菜豆腐煎餅 再現レシピ
recipeId: dh-kimchi-tofu-pancake
description: 涓豆腐の 泡菜豆腐煎餅 を家庭で再現する、シェフおすすめ 向けレシピです。
coverImage: /images/recipes/dh-kimchi-tofu-pancake.svg
servings: 2
prepTime: 25
cookTime: 20
totalTime: 45
difficulty: むずかしい
calories: 720
protein: 34
fat: 36
carbs: 52
category: "シェフおすすめ"
scenarios:
- 夜食
equipment:
- フライヤー
- 鉄板またはフライパン
tags:
- 名店再現
- シェフおすすめ
restaurantSource:
  restaurant: 涓豆腐 Dubu House
  dishName: 泡菜豆腐煎餅
  region: 全台連鎖
intro: 涓豆腐でよく見られる下ごしらえと仕上げの流れを意識した再現レシピです。
steps:
- キムチを細切りまたは粗みじん切りにしておきます。 純豆腐は水切りして崩しておきます。
- ボウルに中力粉、卵、塩、水を混ぜ合わせ、切ったキムチ, 純豆腐, 青ネギを加えて生地を作ります。
- フライパンにごま油を熱し、生地を流し込んで薄く広げます。中火で底がカリッと香ばしくなるまで焼きます。
- 裏返し、フライパンの端からさらに少すごま油を回し入れ、両面がきつね色になるまでサクサクに焼いて切り分けます。
tips:
- 調理前に調味料をよく混ぜ合わせておくことで、均一に味が広がります。
- 熱したフライパンと十分な油で素早く炒めることが、水分を逃がさないコツです。
storage: できれば当日中がおすすめ。保存する場合は具材とスープを分けて冷蔵します。
substitutions:
- キムチ
- 中力粉
relatedIngredients:
- 韓式泡菜
- 嫩豆腐
- 青蔥
customAdditions:
- 完全菜食に変更可
- 刻みネギ
faqs:
- question: 泡菜豆腐煎餅 を店っぽくするコツは？
  answer: スープやタレの濃度を先に整え、最後に具材を加えるのが近道です。
featured: false
publishedAt: "2026-06-03"
updatedAt: "2026-06-03"
ingredients:
- name: キムチ
  amount: "100"
  unit: g
  isCore: true
- name: 純豆腐
  amount: "1"
  unit: パック
  isCore: true
- name: 青ネギ
  amount: "1"
  unit: 本
  isCore: true
seasonings:
- name: 中力粉
  amount: "150"
  unit: g
- name: 卵
  amount: "1"
  unit: 個
- name: 塩
  amount: "0.25"
  unit: 少々
- name: ごま油
  amount: "1"
  unit: 大さじ
---

この再現版は、涓豆腐らしいスープの層とボリューム感を意識して作っています。

