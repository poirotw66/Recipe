---
title: 番茄洋蔥滑蛋
slug: tomato-onion-scrambled-eggs
description: 番茄洋蔥炒蛋一人份，10 分鐘租屋族家常菜，酸甜鹹香適合配白飯或麵包。
coverImage: /images/recipes/tomato-onion-scrambled-eggs.webp
servings: 1
prepTime: 6
cookTime: 8
totalTime: 14
difficulty: 簡單
calories: 265
protein: 13
fat: 17
carbs: 14
category: "家常菜"
scenarios:
- 10 分鐘料理
- 一人料理
equipment:
- 平底鍋
ingredients:
- name: 番茄
  amount: '1'
  unit: 顆
  isCore: true
- name: 洋蔥
  amount: '0.25'
  unit: 顆
  isCore: true
- name: 雞蛋
  amount: '2'
  unit: 顆
  isCore: true
seasonings:
- name: 鹽
  amount: 少許
  unit: ''
- name: 青蔥
  amount: '1'
  unit: 支
- name: 橄欖油
  amount: '1'
  unit: 大匙
tags:
- 滑蛋
- 番茄料理
intro: 番茄和洋蔥的比例平衡後，蛋的香氣會更明顯，整道菜會很耐吃。
steps:
- 番茄 1 顆切塊；洋蔥 1/4 顆切細絲；雞蛋 2 顆打散加鹽少許；青蔥 1 支切蔥花。
- 平底鍋中火熱橄欖油 1 大匙，下洋蔥炒 2 分鐘至微透明出甜。
- 加入番茄塊，中火炒 2 分鐘至略軟、出汁（番茄皮捲曲即可）。
- 將蔬菜推至鍋邊，倒入蛋液，待邊緣凝固後用鍋鏟輕推成嫩蛋塊（約 1 分鐘）。
- 全部翻拌均勻，試味後撒蔥花；蛋液應半凝固帶滑，勿炒太乾。
tips:
- 喜歡蛋更嫩可提早離火。
storage: 蛋料理建議現吃，冷藏後口感會變硬。
substitutions:
- 青蔥可改成少量蒜末。
faqs:
- question: 番茄太酸怎麼辦？
  answer: 可加一小撮糖平衡酸度。
- question: 能做成兩人份嗎？
  answer: 材料加倍即可，建議分兩次炒蛋較好控制熟度。
relatedIngredients:
- 番茄
- 洋蔥
- 雞蛋
featured: false
publishedAt: '2026-06-24'
updatedAt: "2026-07-06"
customAdditions:
- 火腿丁
- 起司絲
- 蔥花
---
這道菜很適合拿來救援晚餐，流程短且幾乎不需要額外備料。
