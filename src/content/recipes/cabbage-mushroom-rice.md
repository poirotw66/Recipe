---
title: 高麗菜菇菇炊飯
slug: cabbage-mushroom-rice
description: 高麗菜菇菇炊飯用電鍋完成，適合省錢備餐與一人份平日主食。
coverImage: /images/recipes/cabbage-mushroom-rice.webp
servings: 3
prepTime: 8
cookTime: 25
totalTime: 33
difficulty: 簡單
calories: 430
protein: 12
fat: 6
carbs: 82
category: "飯麵"
scenarios:
- 省錢料理
- 便當菜
- 10 分鐘料理
equipment:
- 電鍋
ingredients:
- name: 白米
  amount: '2'
  unit: 杯
  isCore: true
- name: 高麗菜
  amount: 1/4
  unit: 顆
  isCore: true
- name: 菇類
  amount: '1'
  unit: 碗
  isCore: true
seasonings:
- name: 醬油
  amount: '1.5'
  unit: 大匙
- name: 鹽
  amount: 少許
  unit: ''
tags:
- 電鍋
- 家常
intro: 把高麗菜和菇類切好鋪進電鍋，就是很穩的備餐主食。
steps:
- 白米 2 杯洗淨瀝至米粒表面微乾；高麗菜 1/4 顆切粗絲；菇類 1 碗撕成小朵洗淨。
- 內鍋放入白米，加水至電鍋「2 杯米」刻度線再少約 1/4 格（高麗菜會出水，水太多飯會軟爛）。
- 將高麗菜與菇類均勻鋪在米上（不要先拌，避免米粒黏在水面下）。
- 均勻淋上醬油 1.5 大匙。
- 外鍋加 1 杯水（各品牌略異，第一次可從 1 杯起試），按下開關。
- 跳起後悶 10 分鐘，再開蓋用飯匙由底部向上翻拌（讓醬油與蔬菜均勻）。
- 以鹽少許試味；米飯應粒粒分明、高麗菜半透明帶甜，即可分裝 3 份。
tips:
- 蔬菜會出水，水量要略減，飯才不會太濕。
storage: 冷藏可保存 2 天，分裝後冷凍可保存 1 週。
substitutions:
- 洋蔥
- 紅蘿蔔
faqs:
- question: 可以加肉嗎？
  answer: 可以加入切丁雞胸肉，但要確認完全熟透。
relatedIngredients:
- 白米
- 高麗菜
- 菇類
featured: false
publishedAt: '2026-05-29'
updatedAt: "2026-07-06"
customAdditions:
- 肉絲
- 蝦仁
- 香菇
---

這道炊飯適合週末一次煮好，平日再搭配蛋或肉類。
