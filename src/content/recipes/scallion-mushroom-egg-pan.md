---
title: 青蔥菇菇蛋煎盤
slug: scallion-mushroom-egg-pan
description: 把菇類煎香後和青蔥蛋液結合，香氣明顯，做法簡潔。
coverImage: /images/recipes/scallion-mushroom-egg-pan.webp
servings: 1
prepTime: 6
cookTime: 8
totalTime: 14
difficulty: 簡單
calories: 260
protein: 15
fat: 17
carbs: 9
category: "家常菜"
scenarios:
- 一人料理
- 10 分鐘料理
equipment:
- 平底鍋
ingredients:
- name: 菇類
  amount: '130'
  unit: 克
  isCore: true
- name: 雞蛋
  amount: '2'
  unit: 顆
  isCore: true
- name: 青蔥
  amount: '1'
  unit: 支
  isCore: false
seasonings:
- name: 鹽
  amount: 少許
  unit: ''
- name: 醬油
  amount: '0.5'
  unit: 小匙
- name: 橄欖油
  amount: '1'
  unit: 大匙
tags:
- 蛋料理
- 菇類料理
intro: 先把菇類表面煎到微焦，再下蛋液，能吃到更濃的菇香。
steps:
- 菇類 130 克切片；雞蛋 2 顆打散加鹽少許；青蔥 1 支切蔥花（留一半最後撒）。
- 平底鍋中火熱橄欖油 1 大匙，下菇類鋪平，不要堆疊，煎 2～3 分鐘至一面金黃再翻面。
- 菇類炒至微焦出香後鋪平，將蔥花拌入蛋液，均勻倒入鍋中。
- 蓋鍋小火悶 2 分鐘至蛋液表面凝固（中心仍略嫩）；或開蓋用鏟輕推成厚蛋餅。
- 淋醬油 0.5 小匙，對折或切塊起鍋；表面應帶蔥香、菇片焦香明顯。
tips:
- 菇類不要一次堆太多，才容易上色。
- 蔬菜下鍋前盡量瀝乾，避免鍋內水分過多而變成燜煮，口感會更清脆。
storage: 可冷藏 1 天，回鍋後口感會偏乾。
substitutions:
- 可加入洋蔥絲增加甜味。
faqs:
- question: 可以不翻面嗎？
  answer: 可以，蓋鍋悶熟也可。
- question: 能配飯嗎？
  answer: 很適合配白飯或糙米飯。
- question: "蔬菜出水時怎麼辦？"
  answer: "先把食材表面水分瀝乾，並分批下鍋保留鍋面溫度，就能減少出水與軟爛。"
relatedIngredients:
- 菇類
- 雞蛋
- 青蔥
featured: false
publishedAt: '2026-07-03'
updatedAt: "2026-07-30"
customAdditions:
- 火腿丁
- 起司絲
- 蔥花
---
青蔥菇菇蛋煎盤適合忙碌平日，食材取得容易，流程清楚，重複製作也穩定。
