---
title: 甘味飯
description: 參考貳樓 Second Floor Cafe 常見做法，以 蜜餞、米飯、台南限定 還原 甘味飯 的餐廳份量與風味層次。
coverImage: /images/recipes/sf-sweet-savory-rice.webp
servings: 1
prepTime: 20
cookTime: 18
totalTime: 38
difficulty: 中等
calories: 640
protein: 28
fat: 28
carbs: 70
category: "限定主餐"
scenarios:
- 一人料理
equipment:
- 平底鍋
- 烤箱
tags:
- 名店還原
- 季節限定
restaurantSource:
  restaurant: 貳樓 Second Floor Cafe
  dishName: 甘味飯
  region: 全台連鎖
intro: 這道 甘味飯 參考貳樓 Second Floor Cafe 常見做法，會先把 蜜餞、米飯、台南限定 分段處理，再依 季節限定 的出餐邏輯完成整盤。
steps:
- 備料：蜜餞 40 公克 切小丁；米飯 適量 蒸好至鬆軟保溫，以鍋鏟鬆開結塊；台南限定 適量 依包裝切好備用。
- 平底鍋小火下橄欖油 1 大匙，輕炒蜜餞 1 分鐘至表面微亮、釋出甘香（注意勿炒焦）。
- 倒入米飯 適量，轉中火翻炒 2 分鐘至米粒外層透油、粒粒分明（炒米）。
- 每次加入 2 大匙熱水，待吸收後再加，推拌 2～3 次約 4 分鐘使米粒略帶濕潤黏性。
- 以海鹽 1/3 小匙、黑胡椒碎 1/4 小匙 平衡甜度，試味調整甘鹹比例。
- 拌入台南限定 適量，再拌炒 1 分鐘使甘鹹風味均勻分布。
- 關火後輕鬆拌勻，避免米粒過碎。
- 盛碗趁熱上桌，還原貳樓台南限定甘味飯的甜鹹層次。
tips:
- 限定品項通常以單一強烈味型作為辨識點，先把那個核心風味做好最重要。
- 若參考的是食記版本，建議用門市常見配菜邏輯補齊整體份量。
storage: 限定品項以現做現吃為佳；若需冷藏，醬汁與主體請分開保存。
substitutions:
- 時蔬
- 白飯
relatedIngredients:
- 蜜餞
- 米飯
- 台南限定
customAdditions: []
faqs:
- question: 想把 甘味飯 做得更像貳樓版本，最重要的是什麼？
  answer: 先把 蜜餞與米飯 的火候抓對，再讓醬汁在最後階段包住主體，整體會更接近 Second Floor Cafe 的厚度與份量感。
featured: false
publishedAt: "2026-06-09"
updatedAt: "2026-07-06"
ingredients:
- name: 蜜餞
  amount: "40"
  unit: 公克
  isCore: true
- name: 米飯
  amount: "適量"
  unit: ""
  isCore: true
- name: 台南限定
  amount: "適量"
  unit: ""
  isCore: true
seasonings:
- name: 橄欖油
  amount: "1"
  unit: 大匙
- name: 海鹽
  amount: "1/3"
  unit: 小匙
- name: 黑胡椒碎
  amount: "1/4"
  unit: 小匙
---
甘味飯 的在家還原版，重點是把 蜜餞、米飯、台南限定 做出分層口感，並保留貳樓常見的 季節限定 大份量出餐感。

只要依照先處理主料、再完成醬汁、最後整盤組裝的順序，就能做出接近 Second Floor Cafe 門市視覺與口味的還原版。


