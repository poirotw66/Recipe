---
title: 黑松露厚切藍帶豬／歐包 Open
description: 參考貳樓 Second Floor Cafe 常見做法，以 歐包、藍帶豬排、黑松露醬、水波蛋 還原 黑松露厚切藍帶豬／歐包 Open 的餐廳份量與風味層次。
coverImage: /images/recipes/sf-black-truffle-cordon-bleu-pork-open.webp
servings: 1
prepTime: 18
cookTime: 14
totalTime: 32
difficulty: 中等
calories: 590
protein: 32
fat: 26
carbs: 38
category: "早午餐"
scenarios:
- 一人料理
- 10 分鐘料理
equipment:
- 平底鍋
- 烤箱
tags:
- 名店還原
- 開放三明治
restaurantSource:
  restaurant: 貳樓 Second Floor Cafe
  dishName: 黑松露厚切藍帶豬／歐包 Open
  region: 全台連鎖
intro: 這道 黑松露厚切藍帶豬／歐包 Open 參考貳樓 Second Floor Cafe 常見做法，會先把 歐包、藍帶豬排、黑松露醬 分段處理，再依 開放三明治 的出餐邏輯完成整盤。
steps:
- 歐包 2 份橫切，切面抹少許奶油，以 180°C 烤箱烤 5～7 分鐘至表面金黃微脆。
- 藍帶豬排 220 公克室溫回溫 10 分鐘，兩面以海鹽與黑胡椒少許調味；平底鍋中火下橄欖油，每面煎 4～5 分鐘至金黃熟透，起鍋靜置 2 分鐘後斜切片。
- 小碗混合黑松露醬 3 大匙、松露醬 3 大匙、橄欖油 1 大匙、海鹽 1/3 小匙、黑胡椒碎 1/4 小匙成松露醬汁。
- 湯鍋燒滾水加少許醋，轉小火攪出漩渦，打入雞蛋 2 顆做出水波蛋（約 3 分鐘，蛋黃流心）。
- 烤好的歐包先抹上 1/3 松露醬汁，依序鋪上藍帶豬排片。
- 放上水波蛋 2 顆，淋剩餘松露醬汁，趁熱上桌。
tips:
- 麵包要先烤到外脆內軟，再疊上溫熱主料，才會有門市開放三明治的對比口感。
- 水波蛋與生菜最後上桌，才能保留蛋黃流心和葉菜的新鮮度。
- 肉類起鍋後先靜置 2～3 分鐘再切，肉汁較不易流失；厚度不同時以最厚處熟度為準。
storage: 建議現做現吃；若要提前準備，麵包與配料請分開保存，食用前再組裝。
substitutions:
- 酸種麵包
- 丹麥麵包
relatedIngredients:
- 歐包
- 藍帶豬排
- 水波蛋
customAdditions:
- 水波蛋加一顆
- 巴薩米克另外放
faqs:
- question: 想把 黑松露厚切藍帶豬／歐包 Open 做得更像貳樓版本，最重要的是什麼？
  answer: 先把 歐包與藍帶豬排 的火候抓對，再讓醬汁在最後階段包住主體，整體會更接近 Second Floor Cafe 的厚度與份量感。
- question: "如何避免主食材變乾？"
  answer: "依厚度調整火候，起鍋後靜置再切；若要保存，完全放涼後密封冷藏並用較低溫回熱。"
featured: false
publishedAt: "2026-06-09"
updatedAt: "2026-07-30"
ingredients:
- name: 歐包
  amount: "2"
  unit: 份
  isCore: true
- name: 藍帶豬排
  amount: "220"
  unit: 公克
  isCore: true
- name: 水波蛋
  amount: "2"
  unit: 顆
  isCore: true
seasonings:
- name: 橄欖油
  amount: "1"
  unit: 大匙
- name: 海鹽
  amount: "1/3"
  unit: 小匙
- name: 黑胡椒碎
  amount: "1/4"
  unit: 小匙
- name: 黑松露醬
  amount: "3"
  unit: 大匙
- name: 松露醬
  amount: "3"
  unit: 大匙
---

黑松露厚切藍帶豬／歐包 Open 的在家還原版，重點是把 歐包、藍帶豬排、黑松露醬 做出分層口感，並保留貳樓常見的 開放三明治 大份量出餐感。

只要依照先處理主料、再完成醬汁、最後整盤組裝的順序，就能做出接近 Second Floor Cafe 門市視覺與口味的還原版。


