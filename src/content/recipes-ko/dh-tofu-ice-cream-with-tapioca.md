---
title: 珍珠豆腐冰淇淋 스타일 레시피
recipeId: dh-tofu-ice-cream-with-tapioca
description: 涓豆腐 珍珠豆腐冰淇淋 를 집에서 재현하는 디저트 레시피입니다.
coverImage: /images/recipes/dh-tofu-ice-cream-with-tapioca.svg
servings: 2
prepTime: 15
cookTime: 12
totalTime: 27
difficulty: 쉬움
calories: 380
protein: 6
fat: 14
carbs: 56
category: "디저트"
scenarios:
- 1인 요리
equipment:
- 프라이팬
- 믹싱 볼
tags:
- 맛집 재현
- 디저트
restaurantSource:
  restaurant: 涓豆腐 Dubu House
  dishName: 珍珠豆腐冰淇淋
  region: 全台連鎖
intro: 涓豆腐에서 흔히 쓰는 준비와 마무리 순서를 참고한 재현 레시피입니다.
steps:
- 비유전자조작 대두을 섞어 디저트 베이스를 만듭니다(호떡 반죽을 발효시키거나, 삶은 대두로 두유를 만듭니다).
- 팬에 식용유나 버터를 두르고 약불에서 노릇하게 구워냅니다(혹은 콩 퓨레를 휘핑하여 부드러운 아이스크림으로 만듭니다).
- 흑당 타피오카 펄을 준비합니다(흑설탕 계피 견과 소를 섞거나, 흑당 타피오카 펄이 졸여질 때까지 졸입니다).
- 두 가지를 결합하여(반죽에 소를 넣어 구워 완성하거나, 두부 아이스크림 위에 흑당 펄을 얹어 완성합니다).
tips:
- 조리 전에 양념장을 골고루 섞어두어야 뭉침 없이 일정한 맛을 낼 수 있습니다.
- 충분히 달군 팬에 센불로 빠르게 볶아내야 채소와 고기의 수분을 가둘 수 있습니다.
storage: 가급적 당일 섭취를 권장하며, 보관 시 재료와 국물을 나누어 냉장하세요.
substitutions:
- 비유전자조작 대두
- 흑설탕
relatedIngredients:
- 非基改黃豆
- 黑糖珍珠
customAdditions:
- 다진 파
- 참깨
faqs:
- question: 珍珠豆腐冰淇淋 를 매장 스타일에 가깝게 만드는 방법은?
  answer: 국물이나 소스 농도를 먼저 맞추고 마지막에 재료를 넣으세요.
featured: false
publishedAt: "2026-06-03"
updatedAt: "2026-06-03"
ingredients:
- name: 비유전자조작 대두
  amount: "150"
  unit: g
  isCore: true
- name: 흑당 타피오카 펄
  amount: "50"
  unit: g
  isCore: true
seasonings:
- name: 흑설탕
  amount: "50"
  unit: g
- name: 소금
  amount: "0.25"
  unit: 약간
---

이 재현 버전은 涓豆腐 특유의 국물 층과 넉넉한 한 접시 구성을 목표로 했습니다.

