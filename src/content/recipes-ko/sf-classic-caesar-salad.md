---
title: 經典凱薩沙拉 스타일 레시피
recipeId: sf-classic-caesar-salad
description: Second Floor Cafe 스타일로 綜合生菜、培根、麵包丁、凱薩醬、莎莎醬 구성을 살린, 매장 분량 기준의 재현 레시피입니다.
coverImage: /images/recipes/sf-classic-caesar-salad.webp
servings: 2
prepTime: 18
cookTime: 18
totalTime: 36
difficulty: 보통
calories: 480
protein: 22
fat: 28
carbs: 38
category: "애피타이저"
scenarios:
- 1인 요리
- 야식
equipment:
- 튀김냄비
- 오븐
tags:
- 맛집 재현
- 애피타이저
restaurantSource:
  restaurant: 貳樓 Second Floor Cafe
  dishName: 經典凱薩沙拉
  region: 全台連鎖
intro: Second Floor Cafe에서 자주 보이는 조리 흐름을 참고해 한 접시 전체의 완성도를 살리는 방식으로 만듭니다.
steps:
- 綜合生菜 120公克、培根 80公克、麵包丁 40公克를 준비하고 채소는 말리며, 따뜻하게 먹을 재료는 따로 익힌다(炸油 600毫升、海鹽 1/2小匙、凱薩醬 2大匙、莎莎醬 2大匙).
- 단백질·곡물을 익혀 표면에 살짝 색을 낸다.
- 큰 그릇에 채소와 따뜻한 재료, 炸油 600毫升、海鹽 1/2小匙、凱薩醬 2大匙、莎莎醬 2大匙를 나눠 넣어 버무린다.
- 견과나 과일을 올려 經典凱薩沙拉 스타일 레시피를 마무리한다.
tips:
- 주재료와 소스를 따로 완성한 뒤 마지막에 합치면 질감 차이가 더 살아납니다.
- 차가운 가니시와 뜨거운 단백질은 마지막에 합쳐야 매장 같은 결과가 납니다.
storage: 가급적 당일 섭취를 권장합니다. 보관할 때는 재료와 소스를 나누어 냉장하고 먹기 직전에 부드럽게 다시 데우세요.
substitutions:
- 冷凍薯條
- 蘿蔓心
relatedIngredients:
- 綜合生菜
- 培根
- 麵包丁
customAdditions: []
faqs:
- question: 經典凱薩沙拉 를 매장 스타일에 가깝게 만드는 핵심은 무엇인가요?
  answer: 주재료의 익힘을 먼저 맞추고 마지막에 소스를 입혀 마무리하면 매장 같은 두께감과 볼륨이 살아납니다.
featured: false
publishedAt: "2026-06-09"
updatedAt: "2026-06-09"
ingredients:
- name: 綜合生菜
  amount: "120"
  unit: 公克
  isCore: true
- name: 培根
  amount: "80"
  unit: 公克
  isCore: true
- name: 麵包丁
  amount: "40"
  unit: 公克
  isCore: true
seasonings:
- name: 炸油
  amount: "600"
  unit: 毫升
- name: 海鹽
  amount: "1/2"
  unit: 小匙
- name: 凱薩醬
  amount: "2"
  unit: 大匙
- name: 莎莎醬
  amount: "2"
  unit: 大匙
---

---
---
---

이 재현 버전은 Second Floor Cafe 특유의 넉넉한 한 접시 구성과 층이 살아 있는 식감을 목표로 했습니다.

주재료 준비, 소스 완성, 마지막 플레이팅 순서를 지키면 매장 스타일에 더 가깝게 만들 수 있습니다.

