---
recipeId: air-fryer-lemon-fish-fillet
title: 에어프라이어 레몬 생선 필레
description: 흰살 생선을 레몬과 허브 향으로 에어프라이어에 구워 살은 부드럽고 상큼하게, 1인분에 딱 맞습니다.
coverImage: /images/recipes/air-fryer-lemon-fish-fillet.webp
servings: 1
prepTime: 8
cookTime: 12
totalTime: 20
difficulty: 쉬움
calories: 280
protein: 36
fat: 12
carbs: 4
category: "메인 요리"
scenarios:
- 에어프라이어 요리
- 1인 요리
equipment:
- 에어프라이어
tags:
- 1인분
- 에어프라이어
- 해산물 요리
tips:
- 두께가 2cm를 넘으면 2분 정도 늘리되, 포크로 쉽게 갈라지는지 꼭 확인하세요.
- 재료 표면의 물기를 닦고 한 겹으로 놓아 열풍이 지나갈 공간을 만들면 고르게 노릇해집니다.
substitutions:
- 연어
- 넙치
relatedIngredients:
- 대구
- 브로콜리
customAdditions: []
ingredients:
- name: 대구
  amount: '1'
  unit: 토막
  isCore: true
- name: 브로콜리
  amount: '80'
  unit: g
  isCore: false
seasonings:
- name: 레몬즙
  amount: '1.5'
  unit: 큰술
- name: 올리브 오일
  amount: '1'
  unit: 큰술
- name: 마늘
  amount: '1'
  unit: 쪽
- name: 소금
  amount: 약간
  unit: ''
- name: 후추
  amount: 약간
  unit: ''
intro: 생선은 오래 굽으면 마르므로 시간은 짧게, 가운데는 촉촉하게 남기는 것이 가장 맛있습니다.
steps:
- 대구 필레 1장을 키친타월로 양면 물기를 완전히 뺀다. 마늘 1쪽을 다진다. 브로콜리 80g을 작은 송이로 씻어 물기를 빼고 키친타월로 눌러 말린다.
- 그릇에 레몬즙 1.5큰술, 올리브 오일 1큰술, 마늘, 소금 약간, 후추를 섞어 절반을 생선 양면에 발라 5분 재운다.
- 에어프라이어를 180°C로 설정하고 3분 예열한다.
- 생선을 바스켓에 평평히 놓고 남은 양념을 위에 바른 뒤 한 겹으로 겹치지 않게 한다. 180°C에서 6분 굽고 살짝 뒤집어 4~5분 더 굽아 표면이 옅게 탈 때까지.
- 포크로 가장 두꺼운 부분을 살짝 갈라 불투명하고 쉽게 갈라지면 완성. 생선을 꺼내 보온한다.
- 바스켓에 브로콜리를 펼쳐 180°C에서 3~4분 굽아 줄기가 약간 부드럽고 아삭함이 남을 때까지(또는 끓는 물에 1분 데친다).
- 생선과 브로콜리를 담고 취향에 따라 레몬즙을 더 뿌려 뜨겁게 낸다.
storage: 바로 드세요. 생선은 냉장 후 재가열 시 쉽게 마를 수 있어 당일 완성을 권합니다.
faqs:
- question: 바스켓에 붙나요?
  answer: 유산지를 깔거나 기름을 바르면 덜 붙고, 뒤집을 때 부드럽게 하면 됩니다.
- question: "재료 두께가 다르면 조리 시간을 어떻게 조절하나요?"
  answer: "표시된 시간에 먼저 확인하고 가장 두꺼운 부분의 익힘을 봅니다. 필요하면 2~3분씩 추가하세요."
featured: false
publishedAt: '2026-06-03'
updatedAt: "2026-07-30"
---

에어프라이어 레몬 생선 필레는 상큼하고 가벼워 집에서도 빠르게 끝내는 생선 요리입니다.
