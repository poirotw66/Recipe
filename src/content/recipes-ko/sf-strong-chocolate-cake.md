---
title: 強的 스타일 레시피
recipeId: sf-strong-chocolate-cake
description: Second Floor Cafe 스타일로 4 吋巧克力蛋糕 + 奶油起司 구성을 살린, 매장 분량 기준의 재현 레시피입니다.
coverImage: /images/recipes/sf-strong-chocolate-cake.webp
servings: 2
prepTime: 20
cookTime: 28
totalTime: 48
difficulty: 보통
calories: 420
protein: 7
fat: 30
carbs: 64
category: "디저트"
scenarios:
- 1인 요리
equipment:
- 오븐
- 믹싱 볼
tags:
- 맛집 재현
- 디저트
restaurantSource:
  restaurant: 貳樓 Second Floor Cafe
  dishName: 強的
  region: 全台連鎖
intro: Second Floor Cafe에서 자주 보이는 조리 흐름을 참고해 한 접시 전체의 완성도를 살리는 방식으로 만듭니다.
steps:
- 4인치 초콜릿 케이크 1개를 냉장고에서 꺼내 실온에서 15분 둡니다. 크림치즈 80g도 가볍게 누르면 들어갈 정도로 부드럽게 합니다. 무염 버터 30g은 작게 썹니다.
- 크림치즈 80g과 설탕 2큰술을 주걱이나 믹서로 저속 섞어 알갱이 없이 매끄럽게 합니다(과하게 거품 내지 않음).
- 긴 칼로 케이크를 가로로 두 층으로 자릅니다(이미 한 층이면 생략). 아래층을 접시에 두고 크림치즈 프로스팅 절반을 바르고 윗층을 가볍게 올립니다.
- 남은 크림치즈 프로스팅을 윗면과 옆면에 고르게 바르고 회전대를 돌리며 오프셋 주걱으로 표면을 정리합니다. 코코아 가루나 초콜릿 가루를 뿌려도 좋습니다.
- 냉장고에서 30분 식혀 크림치즈 층을 굳히고 풍미를 섞습니다.
- 칼을 뜨거운 물에 담갔다 닦고 2등분 또는 부채꼴로 자릅니다. 한 번 자를 때마다 칼을 닦으면 단면이 깔끔합니다.
- 접시에 무염 버터 30g 작은 조각과 설탕 2큰술을 곁들여 취향대로 곁들이게 합니다.
- 케이크가 살짝 식고 크림치즈 층이 부드러울 때 내며, 진한 초콜릿과 치즈 대비로 貳樓 ‘강한’ 디저트 맛을 재현한다.
tips:
- 주재료와 소스를 따로 완성한 뒤 마지막에 합치면 질감 차이가 더 살아납니다.
- 차가운 가니시와 뜨거운 단백질은 마지막에 합쳐야 매장 같은 결과가 납니다.
storage: 가급적 당일 섭취를 권장합니다. 보관할 때는 재료와 소스를 나누어 냉장하고 먹기 직전에 부드럽게 다시 데우세요.
substitutions:
- 鮮奶油
- 香草冰淇淋
relatedIngredients:
- 4 吋巧克力蛋糕
- 奶油起司
customAdditions:
- 바닐라 아이스크림
- 휘핑크림
faqs:
- question: 強的 를 매장 스타일에 가깝게 만드는 핵심은 무엇인가요?
  answer: 주재료의 익힘을 먼저 맞추고 마지막에 소스를 입혀 마무리하면 매장 같은 두께감과 볼륨이 살아납니다.
featured: false
publishedAt: "2026-06-09"
updatedAt: "2026-07-06"
ingredients:
- name: 4 吋巧克力蛋糕
  amount: "1"
  unit: 份
  isCore: true
- name: 奶油起司
  amount: "80"
  unit: 公克
  isCore: true
seasonings:
- name: 無鹽奶油
  amount: "30"
  unit: 公克
- name: 細砂糖
  amount: "2"
  unit: 大匙
---

이 재현 버전은 Second Floor Cafe 특유의 넉넉한 한 접시 구성과 층이 살아 있는 식감을 목표로 했습니다.

주재료 준비, 소스 완성, 마지막 플레이팅 순서를 지키면 매장 스타일에 더 가깝게 만들 수 있습니다.

