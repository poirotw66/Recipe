---
title: 春川辣炒雞 스타일 레시피
recipeId: dh-spicy-stired-fried-chicken
description: 涓豆腐 春川辣炒雞 를 집에서 재현하는 한국식 메인 레시피입니다.
coverImage: /images/recipes/dh-spicy-stired-fried-chicken.webp
servings: 2
prepTime: 22
cookTime: 22
totalTime: 44
difficulty: 보통
calories: 640
protein: 30
fat: 26
carbs: 58
category: "한국식 메인"
scenarios:
- 고단백 요리
equipment:
- 프라이팬
- 냄비
tags:
- 맛집 재현
- 한국식 메인
restaurantSource:
  restaurant: 涓豆腐 Dubu House
  dishName: 春川辣炒雞
  region: 全台連鎖
intro: 涓豆腐에서 흔히 쓰는 준비와 마무리 순서를 참고한 재현 레시피입니다.
steps:
- 닭다리살을 한입 크기로 자르고 양파는 채 썰며 대파는 큼직하게 썹니다.
- 닭다리살에 다진 마늘과 춘천 불닭 소스를 넣어 20분간 재워둡니다.
- 팬에 참기름을 두르고 양파를 볶다가 재워둔 닭고기를 넣어 볶습니다.
- 닭고기가 다 익고 양념이 걸쭉해지면 대파를 넣어 가볍게 볶아 마무리합니다.
tips:
- 반드시 두 번 튀겨내어야 육즙을 가두고 겉을 극대화하여 바삭하게 튀길 수 있습니다.
- 양념에 버무릴 때는 꼭 불을 끄고 빠르게 버무려야 바삭한 튀김옷이 눅눅해지지 않습니다.
storage: 가급적 당일 섭취를 권장하며, 보관 시 재료와 국물을 나누어 냉장하세요.
substitutions:
- 닭다리살
- 닭갈비 소스
relatedIngredients:
- 去骨雞腿肉
- 洋蔥
- 青蔥
customAdditions:
- 다진 파
- 참깨
faqs:
- question: 春川辣炒雞 를 매장 스타일에 가깝게 만드는 방법은?
  answer: 국물이나 소스 농도를 먼저 맞추고 마지막에 재료를 넣으세요.
featured: false
publishedAt: "2026-06-03"
updatedAt: "2026-06-09"
ingredients:
- name: 닭다리살
  amount: "250"
  unit: g
  isCore: true
- name: 양파
  amount: "0.5"
  unit: 개
  isCore: true
- name: 대파
  amount: "1"
  unit: 대
  isCore: true
seasonings:
- name: 닭갈비 소스
  amount: "3"
  unit: 큰술
- name: 참기름
  amount: "1"
  unit: 큰술
- name: 다진 마늘
  amount: "1"
  unit: 큰술
---

이 재현 버전은 涓豆腐 특유의 국물 층과 넉넉한 한 접시 구성을 목표로 했습니다.

