...
export function buildDetailedSfSteps(displayName, categorySlug, ingredients, seasonings) {
  ...
  if (categorySlug === "dessert" || displayName.includes("蛋糕") || displayName.includes("派") || displayName.includes("布朗尼")) {
    if (displayName.includes("起司蛋糕") || lead.includes("起司")) {
      return [
        `Oreo 餅乾 150 克壓碎，拌入融化奶油 50 公克，壓入 6 吋模具底部，冷藏定型 15 分鐘。`,
        `奶油起司 250 克室溫軟化，與細砂糖 60 克、雞蛋 2 顆、鮮奶油 80 毫升以低速攪打至滑順（避免過度打發出氣泡）。`,
        `將起司糊倒入模具，外層包鋁箔；烤盤注入熱水至模具一半高度，烤箱 160°C 水浴烘烤 45～50 分鐘，中心仍略晃動即可。`,
        `關烤箱門悶 30 分鐘，完全冷卻後冷藏 4 小時；切塊時刀沾熱水，每切一刀擦乾。`,
        `盛盤可搭配 ${sauces || "打發鮮奶油或香草冰淇淋"}，冷熱對比更接近餐廳甜點出餐。`
      ];
    }
    if (displayName.includes("布朗尼")) {
      return [
        `黑巧克力 100 克與無鹽奶油 80 克隔水融化；雞蛋 2 顆與細砂糖 80 克打發至略膨。`,
        `將巧克力糊與蛋液混合，篩入低筋麵粉 60 克與可可粉 15 克，拌至無粉粒。`,
        `倒入鋪烘焙紙的模具，170°C 烘烤 22 分鐘，中心略濕潤即可出爐放涼。`,
        `切塊後搭配香草冰淇淋球，淋上 ${sauces || "溫熱巧克力醬"} 上桌。`
      ];
    }
    if (displayName.includes("蜜桃派") || displayName.includes("肉桂")) {
      return [
        `派皮鋪入模具，叉孔後 180°C 先烤 10 分鐘定型；蜜桃切塊與肉桂粉 1 小匙、細砂糖 40 克拌勻。`,
        `將蜜桃餡鋪入派皮，蓋上 lattice 派皮或完整派皮，刷蛋液。`,
        `180°C 烘烤 30～35 分鐘至派皮金黃、餡料冒泡；靜置 15 分鐘再切。`,
        `搭配香草冰淇淋與 ${sauces || "焦糖醬"} 趁溫享用。`
      ];
    }
    return [
      `將 ${lead || "甜點主料"} 與 ${sauces || "奶油、糖"} 依比例備齊；烤箱預熱至 180°C，模具鋪烘焙紙或抹油。`,
      `依序混合乾料與濕料，攪拌至均勻無顆粒，倒入模具約八分滿。`,
      `烘烤 25～30 分鐘至表面定型、中心仍略濕潤，竹籤插入無濕麵糊即可。`,
      `出爐放涼 10 分鐘再脫模，搭配冰淇淋或鮮奶油上桌。`
    ];
  }
